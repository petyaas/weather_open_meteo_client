...
void main() {
  group('OpenMeteoApiClient', () {
    ...
    setUp(() {
      ...
      apiClient = OpenMeteoApi(httpClient: httpClient);
    });

    group('constructor', () {
      test('does not require an httpClient', () {
        expect(OpenMeteoApi(), isNotNull);
      });
    });
  ...
  "results": [
    ...
    group('getWeather', () {
      ...
      test('makes correct http request', () async {
        ...
        when(() => httpClient.get(any())).thenAnswer((_) async => response);
        try {
          await apiClient.getWeatherByCoordinate(latitude: latitude, longitude: longitude);
        } catch (_) {}
        verify(
              () => httpClient.get(
      ...
      test('throws WeatherRequestFailure on non-200 response', () async {
        ...
        when(() => httpClient.get(any())).thenAnswer((_) async => response);
        expect(
              () async => apiClient.getWeatherByCoordinate(
            latitude: latitude,
            longitude: longitude,
          ),
...
"time": "2022-09-12T01:00"
        ...
        when(() => httpClient.get(any())).thenAnswer((_) async => response);
        final actual = await apiClient.getWeatherByCoordinate(
          latitude: latitude,
          longitude: longitude,
        );
        expect(
          actual,
          isA<Weather>()
              .having((w) => w.current_weather!.temperature, 'temperature', 15.3)
              .having((w) => w.current_weather!.weathercode, 'weatherCode', 63.0),
        );
      });
    });
  });
